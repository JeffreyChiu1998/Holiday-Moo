...
const AddBucketItemModal = ({
  ...
  const getChangedFields = () => {
    const changedFields = [];
    if (formData.name.trim() !== "") changedFields.push("Activity Name");
    if (formData.type !== "activity") changedFields.push("Type");
    if (formData.country.trim() !== "") changedFields.push("Country");
    if (formData.city.trim() !== "") changedFields.push("City");
    if (formData.location.trim() !== "") changedFields.push("Location");
    if (formData.estimatedCost.trim() !== "")
      changedFields.push("Estimated Cost");
    if (formData.openHours.trim() !== "") changedFields.push("Open Hours");
    if (formData.link.trim() !== "") changedFields.push("Website Link");
    if (formData.description.trim() !== "") changedFields.push("Description");
    if (formData.imgUrl.trim() !== "") changedFields.push("Image URL");
    if (searchQuery.trim() !== "") changedFields.push("Search Query");
    ...
    >
      ...
      >
        ...
        <div className="add-item-content">
          {/* Google Search Section - Only show when adding new items */}
          {!isEditing && (
            <>
          ...
          <form onSubmit={handleSubmit} className="add-item-form">
            <h3>{isEditing ? "Edit Item Details" : "Add Manually"}</h3>
