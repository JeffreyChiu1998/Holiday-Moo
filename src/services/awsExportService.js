...
class AWSExportService {
  ...
  isAvailable() {
    return (
      this.enabled && this.apiUrl && this.apiUrl !== "http://localhost:5000"
    );
  }
  ...
  async healthCheck() {
    if (!this.isAvailable()) {
      return { status: "disabled", message: "Service not configured" };
    }
    ...
    try {
      ...
      return {
        ...
        message: response.ok ? "Service is running" : "Service returned error",
      };
    } catch (error) {
      return {
        status: "error",
